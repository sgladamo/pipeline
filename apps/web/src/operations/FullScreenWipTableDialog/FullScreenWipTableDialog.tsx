...
import {
  getLastColumnName,
  getLastColumnValue,
} from "operations/OperationsDashboardView/OperationsDashboardView";
...
interface FullScreenWipTableDialogProps {
...
function FullScreenWipTableDialog(props: FullScreenWipTableDialogProps) {
  const { handleDialogClose, table, ops, setOps, editCells } = props;
  ...
  function handleCellChange(
    ...
    if (table?.name === "Assembly Cells" && op.iMachine !== newCell) {
      updateJobCell(op.job as string, newCell);
      let allOpsCopy: WipCurrentOp[] = JSON.parse(JSON.stringify(ops));
      allOpsCopy = allOpsCopy.filter((o) => o.job !== op.job);
      setOps(allOpsCopy);
    } else if (
      table?.name === "Trolley Storage" &&
      op.nextWorkCentreIMachine !== newCell
    ) {
      updateJobCell(op.job as string, newCell);
      let allOpsCopy: WipCurrentOp[] = JSON.parse(JSON.stringify(ops));
      allOpsCopy = allOpsCopy.filter((o) => o.job !== op.job);
              ...
              return table?.name === op.workCentre ||
                ...
                >
                  ...
                  {editCells && loggedIn ? (
                    ...
                    <DefaultTableCell>
                      {op.workCentre === "ASSY01"
                        ? op.iMachine
                        : getLastColumnValue(table as DashboardTable, op)}
                    </DefaultTableCell>
                  )}
                </GreenTableRow>
              ) : (
...
export default FullScreenWipTableDialog;
